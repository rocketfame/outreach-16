...
This file gives Codex a fast, accurate mental model of the project so any new session is productive immediately. Keep it updated when architecture changes.
...
### Language
- **All UI strings must be in English** — the user is Ukrainian but the product is English-only. Codex responds in Ukrainian in chat but writes English in code.
...
- Automation API requires `Authorization: Bearer <AUTOMATION_API_KEY>` and never publishes directly; it only returns generated article payloads
...
Graphify is available as a project-scoped Codex skill in `.codex/skills/graphify/`.
Use it manually for codebase navigation and relationship queries when a local `graphify-out/graph.json` exists.

Rules:
- Graphify hooks are intentionally disabled. Do not run `graphify claude install`, `graphify hook install`, or any install command that registers PreToolUse/tool hooks unless the user explicitly approves that trust surface.
- In Codex, the preferred invocation is `$graphify`. From the terminal, use the local CLI when installed, e.g. `.graphify-venv/bin/graphify .`.
- For codebase questions, first run `graphify query "<question>"` when `graphify-out/graph.json` exists. Use `graphify path "<A>" "<B>"` for relationships and `graphify explain "<concept>"` for focused concepts.
- `graphify-out/` is a generated local artifact and is ignored by git. Rebuild it locally when needed rather than committing it.
