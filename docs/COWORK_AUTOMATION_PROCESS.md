...
**Рішення 2026-09-10: не ускладнювати.** Основний флоу — повна гуманізація
одразу в джобі: `mode:"human"`, `humanizer:"undetectable"` (або `auto`),
`maxCostUsd: 1`. Живий тест негуманізованого BetterWords-драфту (672 слова):
власний score Undetectable 39.8 (human), але симуляції GPTZero / Copyleaks /
ZeroGPT / Writer / Sapling — 100 % AI. Отже негуманізований текст редакторам
не віддаємо, а вибірковий цикл лише додає кроків.

`detect` і `humanize` лишаються для редакторського етапу: перегуманізувати
абзац після ручних правок або перевірити фінальну статтю перед здачею
(детектити ЦІЛУ статтю; короткі абзаци до ~150 слів завжди показують AI і
як сигнал не годяться).
