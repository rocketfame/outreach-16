...
- `POST /api/automation/generate` — async blog autopilot generation job (Bearer `AUTOMATION_API_KEY`). Повертає `202 { status:"queued", jobId, position, etaSeconds }` — джоба реально стає в FIFO-чергу (KV list `automation:queue`), а не відхиляється пост-фактум.
  ...
  - `imageStyle` — optional, id image box пресета (`lib/imageBoxPrompts.ts`, 28 активних); пінить конкретний стиль, невідомий id → 400 + `allowed[]`. `imageStyle` без `image:true` → 400
  - `excludeImageStyles` — optional, масив id для виключення (batch de-dup: оркестратор акумулює `meta.imageStyle` з готових джоб і передає список у наступні). Виключити всі 28 → 400
  - `meta.imageStyle` — id використаного пресета в результаті (selection: uniform random по невиключених, stateless per job)
