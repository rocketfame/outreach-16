...
6. Перед done проходять integrity/orthography/brand guards. Done response повертає body-only `contentHtml`, `cover.base64`, `meta.language`, `meta.billingSource:"api"` та `quotaRemaining:null`; публікація залишається на стороні orchestrator/CMS
